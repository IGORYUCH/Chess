...
def check_opponent_king_check(figures, opponent_figures):# Проверяет, бьет ли игрок короля своего оппонента
    for opponent_figure in opponent_figures:
        if field[opponent_figure[1]][opponent_figure[0]] == 'k':
            opponent_king = opponent_figure[:]
            if opponent_king in white_figures:
                break
    for figure in figures:
        admissible = check_positions(figure, figures, opponent_figures)
...
def check_pawn(cell, opponent_figures): # Проверяет допустимые ячейки поля для хода пешкой
    admissible = []
...
def check_king(cell, opponent_figures): # Проверяет допустимые поля ячейки для хода королем
    admissible = []
...
def check_bishop(cell, opponent_figures): # Проверяет допустимые ячейки поля для хода ладьей
    admissible = []
...
def check_knight(cell, opponent_figures): # Проверяет допустимые ячейки поля для хода конем
    admissible = []
...
def check_rook(cell, opponent_figures): # Проверяет допустимые ячейки поля для хода ладьей
    admissible = []
...
def check_queen(cell, opponent_figures): # Проверяет допустимые ячейки поля для хода ферзем
    admissible = []
    for x,y in zip(range(cell[0]+1, 8), range(cell[1]-1, -1, -1)):
        code = check_cell([x, y], opponent_figures)
        if code == 1:
...
def exclude_check_unprotected_positions(admissible, player_figures, opponent_figures): # Исключает из допустимых для хода ячеек поля те, которые не защищают короля от шаха
    for admissible_pos in admissible:
        player_figures_copy = copy.deepcopy(player_figures)
        player_figures_copy[player_figures_copy.index(selected_figure)] = admissible_pos[:]
        future_check = check_opponent_king_check(opponent_figures, white_figures_copy)
        if future_check:
            admissible.remove(admissible_pos)
    return admissible
...
def check_positions(cell, player_figures, opponent_figures):
    ...
    if figure == 'p':
        ...
        if player_figures == white_figures and white_check:
            admissible = exclude_check_unprotected_positions(admissible, player_figures, opponent_figures)
        elif player_figures == black_figures and black_check:
            admissible = exclude_check_unprotected_positions(admissible, player_figures, opponent_figures)
    elif figure == 'r':
                ...
                if selected_cell in white_figures and white_step:
                    ...
                    if selected_figure != None and white_step: # Если выбрана фигура и ячейка
                        if selected_cell in black_figures and selected_cell in admissible_positions:
                            ...
                            black_check = check_opponent_king_check(white_figures, black_figures)
                            white_check = check_opponent_king_check(black_figures, white_figures)
